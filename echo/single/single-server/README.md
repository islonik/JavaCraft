...
`single-server` is a selector-based non-blocking echo server that serves many clients on one server thread.

## How it is structured

- `my.javacraft.echo.single.server.SingleServer`
...
- `my.javacraft.echo.single.server.SingleServerApplication`
  - CLI entry point.
...
```bash
mvn -pl echo/single/single-server test
```
