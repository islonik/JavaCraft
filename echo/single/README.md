...
- [single-client](single-client/README.md): non-blocking client and message sender/listener flow.
- [single-server](single-server/README.md): selector-based single-thread server implementation.
- [single-testing](single-testing/README.md): Cucumber functional, load, and benchmark scenarios.

## Cucumber Performance Benchmark

Performance scenarios are implemented in `single-testing/src/test/resources/features/benchmark.feature`.

1. SingleServer + SingleClient benchmark:
```bash
...
```text
[PERF][FINAL] average execution time for single benchmark case:
[PERF][FINAL] 1) SINGLE_SERVER+SINGLE_CLIENT average: 0.352 s, throughput=28409.09 msg/s, avg=0.0352 ms/msg, delta=0.000 s (+0.00% vs fastest)
[PERF][FINAL] total measured execution time: 1.056 s (single test)
[PERF][FINAL] total benchmark scenario time (warmups + measured): 1.742 s (single test)
```

### Why This Is Expected

- Single server and single client use one fixed architecture combination, so benchmark summary contains exactly one ranked result.
