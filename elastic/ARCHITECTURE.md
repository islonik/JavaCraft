...
<sub>[Back to Elasticsearch](README.md)</sub>
...
## 1. Goal
<sub>[Back to top](#elastic-architecture)</sub>

`elastic` packages a small Elasticsearch-centered system with two related concerns:
...
|---|---|
| `elastic-api` | shared request/response models, enums, validation, API constants |
| `elastic-app.controller` | HTTP endpoints for admin, search, posts, votes, and rankings |
| `elastic-app.service` | business logic, Elasticsearch orchestration, metadata loading, scheduler cleanup |
| `elastic-app.service.query` | query builders for wildcard, fuzzy, interval, and span searches |
| `elastic-app.service.ranking` | read-side ranking queries over the `posts` index |
| `elastic-app.config` | Elasticsearch client creation, retry/backoff transport, security, scheduler defaults |
| `elastic-testing` | Cucumber scenarios, downloader support, and Testcontainers-based integration coverage |
...
- metadata comes from `elastic-app/src/main/resources/metadata.json`
...
Application-side Elasticsearch access is configured in `elastic-app` with:
